...
getPrice = function() {
    ...
    var coupon = select.options[select.selectedIndex].textContent;
    var hideField = document.querySelector(".coupon-apply");
    //var coupon = document.getElementById("coupon").value;
    var discount = select.options[select.selectedIndex].value; //Descuento aplicado 
	var category = select.options[select.selectedIndex].id; //Descuento aplicado 
    var i;
    ...
    if(coupon){ //Si se aplica el cupon existente
        for(i=0; i<cartItems.length; i++){ //Recorremos el arreglo del carrito
            if(cartItems[i].category === category){ //Si la categoria del item es "soccer"
                ...
                cartHTML();
                updateShoppingCartTotal();
   				alert("Coupon applied! Discount: "+(discount)+"%" + " for " + (category))
				hideField.setAttribute("hidden", true); //Oculta el campo del cupon una vez se aplique alguno 
            }
        }

     
		alert("Coupon not valid for this articles" + (category))
        //hideField.setAttribute("hidden", true); //Oculta el campo del cupon una vez se aplique alguno 
        //hideField.remove();
